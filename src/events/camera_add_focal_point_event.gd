class_name CameraAddFocalPointEvent
extends LevelEvent
...
func _apply_focal_point() -> void:
	if not is_instance_valid(focal_point):
		push_warning("CameraAddFocalPointEvent: focal_point is not set or invalid")
		return
	var rig := _find_camera_rig()
	if rig == null:
		push_warning("CameraAddFocalPointEvent: CameraFollowRig not found")
		return
	rig.add_focal_point(focal_point)

...
const MIN_CARRIER_DISTANCE := 5.5
const MAX_CARRIER_DISTANCE := 7.0
## Min weapon-origin movement per physics frame for a dash to count as a real strike.
...
## actually translated the weapon (i.e. wasn't fully blocked by carrier-distance clamping).
## Read by hit detection (e.g. Area3D on this node) to decide whether contacts deal damage.
...
func _physics_process(_delta: float) -> void:
	...
	# DebugDraw3D.draw_sphere(src.global_position)
	# DebugDraw3D.draw_sphere(tgt.global_position, 0.5, Color.RED)
	_set_look_direction()
